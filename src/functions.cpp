...
void LoRatask(void* parameter){
    ...
    for (;;) {
        ...
        // if (currentMillis - lastBroadcastTime > 5000) {  // Every 30 seconds
        //     broadcastPresence();
        ...
        // if (currentMillis - lastCheckTime > 5000) {  // Every 10 seconds
        //     checkNodeActivity();
...
void activeState() {
  if(activateRelayonce) {
      ...
      digitalWrite(RLYPIN, LOW); // turn on the relay
    }

  const char* activeMsg = "Activated";
...
void inactiveState() {
  digitalWrite(RLYPIN, HIGH); // turn off the relay

  const char* inactiveMsg = "Deactivated";
